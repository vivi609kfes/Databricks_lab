...
tableName = 'self_disclosure_targets_fact'
...
windowSpec = Window.orderBy('company_id','esgpedia_target_id','action_published_date')
...
mySchema = StructType(
    [
        ...
        StructField("esgpedia_target_id"                   ,StringType(),True),
        StructField("self_disclosure_target_framework"     ,StringType(),True),
        StructField("action"                               ,StringType(),True),
        StructField("action_status"                        ,StringType(),True),
        StructField("action_term_status"                   ,StringType(),True),
        StructField("action_published_date"                ,TimestampType(),True),
        StructField("commitment_type"                      ,StringType(),True),
        StructField("commitment_deadline"                  ,TimestampType(),True),
        StructField("commitment_extension_removal_reason"  ,StringType(),True),
        StructField("target_type"                          ,StringType(),True),
        StructField("target_subtype_1"                     ,StringType(),True),
        StructField("target_subtype_2"                     ,StringType(),True),
        StructField("target_scope"                         ,StringType(),True),
        StructField("target_value"                         ,StringType(),True),
        StructField("target_classification"                ,StringType(),True),
        StructField("target_statement"                     ,StringType(),True),
        StructField("target_base_year"                     ,IntegerType(),True),
        StructField("target_year"                          ,IntegerType(),True),
...
create_temp=f"""create table if not exists {rds_credential.get('dbname')}.{tableName_temp} 
            (
                ...
                company_name_wip                        varchar(256),
                esgpedia_target_id                      varchar(256),
                self_disclosure_target_framework        varchar(256),
                action                                  varchar(256),
                action_status                           varchar(256),
                action_term_status                      varchar(28),
                action_published_date                   Datetime,
                commitment_type                         varchar(256),
                commitment_deadline                     Datetime,
                commitment_extension_removal_reason     varchar(256),
                target_type                             varchar(256),
                target_subtype_1                        varchar(256),
                target_subtype_2                        varchar(256),
                target_scope                            varchar(256),
                target_value                            varchar(256),
                target_classification                   varchar(256),
                target_statement                        text,
                target_base_year                        integer,
                target_year                             integer,
                id                                      integer
                ,PRIMARY KEY (id)
            )
