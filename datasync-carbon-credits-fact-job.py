...
## Read Data from Databricks env
tableName = 'carbon_credits_fact'
...
windowSpec = Window.orderBy('company_id','credit_vintage_start_date')
df_target = df.withColumn('id',row_number().over(windowSpec))
print(rds_credential.get('dbname'))
...
mySchema = StructType(
    [
        ...
        StructField("credit_serial_number"                      ,StringType(),True),
        StructField("esgpedia_carbon_credit_id"                 ,StringType(),True),
        ...
        StructField("original_credit_status"                    ,StringType(),True),
        StructField("esgpedia_credit_status"                    ,StringType(),True),
        StructField("credit_quantity"                           ,IntegerType(),True),
        StructField("credit_vintage_start_date"                 ,TimestampType(),True),
        StructField("credit_vintage_end_date"                   ,TimestampType(),True),
        StructField("credit_issuance_date"                      ,TimestampType(),True),
        StructField("credit_retirement_date"                    ,TimestampType(),True),
        StructField("credit_retirement_details"                 ,StringType(),True),
        StructField("credit_retirement_beneficiary"             ,StringType(),True),
        StructField("credit_retirement_beneficiary_type"        ,StringType(),True),
        StructField("credit_retirement_beneficiary_company_id"  ,StringType(),True),
...
create_temp=f"""create table if not exists {rds_credential.get('dbname')}.{tableName_temp} 
            (
                ...
                ,credit_serial_number                      text
                ,esgpedia_carbon_credit_id                 varchar(256)
                ,carbon_credit_framework                   varchar(41)
                ,project_id                                varchar(256)
                ,project_name                              text
                ,project_type                              text
                ,project_country_name                      varchar(256)
                ,project_country_code                      varchar(256)
                ,original_credit_status                    varchar(256)
                ,esgpedia_credit_status                    varchar(256)
                ,credit_quantity                           bigint
                ,credit_vintage_start_date                 Datetime 
                ,credit_vintage_end_date                   Datetime 
                ,credit_issuance_date                      Datetime 
                ,credit_retirement_date                    Datetime 
                ,credit_retirement_details                 text
                ,credit_retirement_beneficiary             text
                ,credit_retirement_beneficiary_type        text
                ,credit_retirement_beneficiary_company_id  varchar(256)
                ,id                                        integer
                ,PRIMARY KEY (id)
            )
            ;
