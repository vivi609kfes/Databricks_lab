...
windowSpec = Window.orderBy('company_id','original_membership_start_date')
...
mySchema = StructType(
    [
        ...
        StructField("original_membership_id"                         ,StringType(),True),
        StructField("esgpedia_membership_id"                         ,StringType(),True),
        StructField("membership_framework"                           ,StringType(),True),
        StructField("membership_sector"                              ,StringType(),True),
        StructField("membership_type"                                ,StringType(),True),
        StructField("membership_level"                               ,StringType(),True),
        StructField("original_membership_status"                     ,StringType(),True),
        StructField("esgpedia_membership_status"                     ,StringType(),True),
        StructField("original_membership_start_date"                 ,TimestampType(),True),
        StructField("original_membership_end_date"                   ,TimestampType(),True),
        StructField("esgpedia_membership_start_date"                 ,TimestampType(),True),
        StructField("esgpedia_membership_expiry_date"                ,TimestampType(),True),
...
create_temp=f"""create table if not exists {rds_credential.get('dbname')}.{tableName_temp} 
            (
                ...
                ,original_membership_id          varchar(256)
                ,esgpedia_membership_id          varchar(256)
                ,membership_framework            varchar(50)
                ,membership_sector               varchar(50)
                ,membership_type                 varchar(256)
                ,membership_level                varchar(384)
                ,original_membership_status      varchar(256)
                ,esgpedia_membership_status      varchar(30)
                ,original_membership_start_date  timestamp
                ,original_membership_end_date    timestamp
                ,esgpedia_membership_start_date  timestamp
                ,esgpedia_membership_expiry_date timestamp
                ,id                                        integer
                ,PRIMARY KEY (id)
            )
            ;
