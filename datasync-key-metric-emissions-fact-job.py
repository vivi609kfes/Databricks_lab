...
tableName = 'key_metric_emissions_fact'
...
windowSpec = Window.orderBy('company_id','esgpedia_key_metric_emissions_id')
...
mySchema = StructType(
    [
        ...
        StructField("esgpedia_key_metric_emissions_id"                 ,StringType(),True),
        StructField("key_metric_emissions_year"                        ,StringType(),True),
        StructField("key_metric_emissions_metric_name"                 ,StringType(),True),
        StructField("key_metric_emissions_category"                    ,StringType(),True),
        StructField("key_metric_emissions_amount"                      ,DecimalType(18,6),True),
        StructField("key_metric_emissions_unit"                        ,StringType(),True),
        StructField("key_metric_emissions_ratio_scope"                 ,StringType(),True),
        StructField("key_metric_emissions_ratio_gases"                 ,StringType(),True),
        StructField("key_metric_emissions_source"                      ,StringType(),True),
        StructField("to_show"                                          ,BooleanType(),True),
        StructField("is_calculated"                                    ,BooleanType(),True),
...
create_temp=f"""create table if not exists {rds_credential.get('dbname')}.{tableName_temp} 
            (
                ...
                ,esgpedia_key_metric_emissions_id           varchar(256)
                ,key_metric_emissions_year                  varchar(256)
                ,key_metric_emissions_metric_name           varchar(256)
                ,key_metric_emissions_category              varchar(256)
                ,key_metric_emissions_amount                decimal(18,6)
                ,key_metric_emissions_unit                  varchar(256)
                ,key_metric_emissions_ratio_scope           varchar(256)
                ,key_metric_emissions_ratio_gases           varchar(256)
                ,key_metric_emissions_source                varchar(256)
                ,to_show                                    Boolean
